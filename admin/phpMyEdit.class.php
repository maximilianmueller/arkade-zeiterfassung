...
function '.$this->js['prefix'].'filter_handler(theForm, theEvent)
{
	...
	function display_button($name, $position = 'up') /* {{{ */
	{
		...
		// when graphic links are used, some buttons dont make sense, because the actions are available in each line
		if (in_array($name, array('view','change','copy','delete')) && $this->nav_graphic_links())
		  return "";
